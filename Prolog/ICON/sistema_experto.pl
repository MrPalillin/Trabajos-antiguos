...
anguloRecto(X):-ask(anguloRecto,'¿La figura posee anguls rectos(si,no)?',X).
ladosParalelos(X):-ask(ladosParalelos,'¿Cuantos lados paralelos tiene la figura(0,2,4)?',X).

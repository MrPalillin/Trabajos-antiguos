...
void actualiza( float *part, int k, int pos, float energia,int gIni[],int rank ) {
...
int main(int argc, char *argv[]) {
	...
	float *part=(float*)malloc(sizeof(float)*layer_size);

	for( k=0; k<layer_size; k++ ) layer[k] = 0.0f;
	for( k=0; k<layer_size; k++ ) layer_copy[k] = 0.0f;
	for( k=0; k<layer_size; k++ ) part[k] = 0.0f;
	
	for( i=0; i<num_storms; i++) {

		/* 4.1. Suma energia de impactos */
		/* Para cada particula */
		MPI_Scatterv(layer,gTam,gIni,MPI_FLOAT,part,gTam[rank],MPI_FLOAT,0,MPI_COMM_WORLD);
		for( j=0; j<storms[i].size; j++ ) {
			...
			for( k=0; k<gTam[rank]; k++ ) {
				/* Actualizar posicion */
				actualiza( part, k, posicion, energia, gIni, rank );
			}
		}

		MPI_Gatherv(part,gTam[rank],MPI_FLOAT,layer,gTam,gIni,MPI_FLOAT,0,MPI_COMM_WORLD);

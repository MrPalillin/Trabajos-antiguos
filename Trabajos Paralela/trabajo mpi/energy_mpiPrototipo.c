...
void actualiza( float *layer, int k, int pos, float energia,int gIni[],int rank ) {
	/* 1. Calcular valor absoluto de la distancia entre el
		punto de impacto y el punto k de la capa */

	
	int distancia = pos - (k+gIni[rank]);
...
int main(int argc, char *argv[]) {
	...
	float *layer = (float *)malloc( sizeof(float) * gTam[rank] );
	float *layer_copy = (float *)malloc( sizeof(float) * gTam[rank] );
	float max_copia[num_storms],max_original[num_storms];
	if ( layer == NULL || layer_copy == NULL ) {
		fprintf(stderr,"Error: Allocating the layer memory\n");
		exit( EXIT_FAILURE );
	}
	for( k=0; k<gTam[rank]; k++ ) layer[k] = 0.0f;
	...
	for( i=0; i<num_storms; i++) {
		...
		for( j=0; j<storms[i].size; j++ ) {
			...
			for( k=0; k<=gTam[rank]; k++ ) {
				/* Actualizar posicion */
				actualiza( layer, k, posicion, energia,gIni,rank );
			}
		}
		...
		for( k=1; k<gTam[rank]-1; k++ ) {
			...
			if ( layer[k] > layer[k-1] && layer[k] > layer[k+1] ) {
				if ( layer[k] > max_copia[i] ) {
					max_copia[i] = layer[k];
					max_original[i] = layer[k];
					posiciones[i] = k+gIni[rank];
					printf("%i\n",rank);
				}
			}
	...
	MPI_Reduce(max_copia,maximos,num_storms,MPI_FLOAT,MPI_MAX,ROOT_RANK,MPI_COMM_WORLD);
	
	//Buscar la posicion,y enviarla a root

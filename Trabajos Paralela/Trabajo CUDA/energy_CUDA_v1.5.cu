...
__global__ void kernel4(float *layerGPU,float *tmp,int layer_size,int *pos){
	int idGlobal = threadIdx.x+(blockDim.x*threadIdx.y)+(blockDim.x*blockDim.y*blockIdx.x);
	if(idGlobal > layer_size-1) return;
	if(idGlobal != 0 && idGlobal != layer_size-1){
...
__global__ void kernel5(float *layerGPU,float *maximosGPU,int i,int layer_size,float *tmp,int tam,int *pos){
	...
	if(idGlobal < tam/2){
		if(tmp[idGlobal] < tmp[idGlobal+(tam/2)] && !(fabs(tmp[idGlobal] - tmp[idGlobal+(tam/2)])<0.000001)){
			tmp[idGlobal] = tmp[idGlobal+(tam/2)];
			pos[idGlobal] = pos[idGlobal+(tam/2)];
		}
	}

	if(tam%2 != 0 && idGlobal == 0 && tmp[0] < tmp[tam-1] && !(fabs(tmp[0] - tmp[tam-1])<0.000001)){
		tmp[0] = tmp[tam-1];
		pos[0] = pos[tam-1];
	}
	__syncthreads();
...
__global__ void kernel6(float *maximosGPU,int *posicionesGPU,int i,float *tmp,int *pos){
	maximosGPU[i] = tmp[0];
	posicionesGPU[i] = pos[0];
}

__global__ void kernel7(float *tmp,int *pos){
	int idGlobal = threadIdx.x+(blockDim.x*threadIdx.y)+(blockDim.x*blockDim.y*blockIdx.x);
	tmp[idGlobal] = 0;
...
int main(int argc, char *argv[]) {
	...
	for( i=0; i<num_storms; i++) {
		...
		kernel4<<<numBlocks,numThreads>>>(layerGPU,tmpGPU,layer_size,posGPU);

		for(int tam = layer_size-2; tam > 1; tam = tam/2){
			kernel5<<<numBlocks,numThreads>>>(layerGPU,maximosGPU,i,layer_size,tmpGPU,tam,posGPU);
		}

		kernel6<<<1,1>>>(maximosGPU,posicionesGPU,i,tmpGPU,posGPU);

		kernel7<<<numBlocks,numThreads>>>(tmpGPU,posGPU);

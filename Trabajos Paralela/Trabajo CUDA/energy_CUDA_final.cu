...
__global__ void relajacionActualiza(float* layerGPU,float* layerCopyGPU,int layer_size){
	int idGlobal = threadIdx.x+blockDim.x*blockIdx.x;	if(idGlobal > layer_size-1) return;
...
int main(int argc, char *argv[]) {
	...
	if(errorLayerGPU != cudaSuccess || errorLayerCopyGPU != cudaSuccess || errorMaxGPU != cudaSuccess || errorPosGPU != cudaSuccess)
		printf("Error en la creación de array en GPU: %s, %s, %s, %s\n",cudaGetErrorString(errorLayerGPU),cudaGetErrorString(errorLayerCopyGPU),cudaGetErrorString(errorMaxGPU),cudaGetErrorString(errorPosGPU));

	cudaError_t errorIni = cudaMalloc(&ini,sizeof(float));
	cudaError_t errorFin = cudaMalloc(&fin,sizeof(float));

	if(errorIni != cudaSuccess || errorFin != cudaSuccess)
		printf("Error en la creación de extremos del array: %s, %s\n",cudaGetErrorString(errorIni),cudaGetErrorString(errorFin));
	...
	if(errorTmp!= cudaSuccess || errorPos != cudaSuccess)
		printf("Error en la creación de extremos del array: %s, %s,\n",cudaGetErrorString(errorTmp),cudaGetErrorString(errorPos));
	...
	for( i=0; i<num_storms; i++) {
		...
		for( j=0; j<storms[i].size; j++ ) {
			...
			actualiza<<<numBlocks,threads_per_block>>>(layerGPU,posicion,energia,layer_size);			
		}

		relajacionCopia<<<numBlocks,threads_per_block>>>(layerGPU,layerCopyGPU,layer_size);
		
		relajacionActualiza<<<numBlocks,threads_per_block>>>(layerGPU,layerCopyGPU,layer_size);

		copiaAtmp<<<numBlocks,threads_per_block>>>(layerGPU,tmpGPU,layer_size,posGPU,ini,fin);

		for(int tam = layer_size-2; tam > 1; tam = tam/2){
			reduccion<<<numBlocks,threads_per_block>>>(layerGPU,maximosGPU,i,layer_size,tmpGPU,tam,posGPU);
		}
	...
	if(errorHostCpyLayer != cudaSuccess || errorHostCpyMax != cudaSuccess || errorHostCpyPos != cudaSuccess)
		printf("Error en la transferecias de arrays al host: %s, %s, %s\n",cudaGetErrorString(errorHostCpyLayer),cudaGetErrorString(errorHostCpyMax),cudaGetErrorString(errorHostCpyPos));
	...
	if(freeLayer != cudaSuccess || freeLayerCopy != cudaSuccess || freeMax != cudaSuccess || freePos != cudaSuccess)
		printf("Error en la liberacion de memoria de arrays: %s, %s, %s,%s\n",cudaGetErrorString(freeLayer),cudaGetErrorString(freeLayerCopy),cudaGetErrorString(freeMax),cudaGetErrorString(freePos));
	...
	if(freeTmp != cudaSuccess || freePosTmp != cudaSuccess || freeIni != cudaSuccess || freeFin != cudaSuccess)
		printf("Error en la liberacion de memoria de arrays auxiliares: %s, %s, %s,%s\n",cudaGetErrorString(freeTmp),cudaGetErrorString(freePosTmp),cudaGetErrorString(freeIni),cudaGetErrorString(freeFin));

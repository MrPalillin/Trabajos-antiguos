...
__global__ void kernel3(float* layerGPU,float* layerCopyGPU,int layer_size){
	int idGlobal = threadIdx.x+(blockDim.x*threadIdx.y)+(blockDim.x*blockDim.y*blockIdx.x);
	if(idGlobal > layer_size-1) return;
...
__global__ void kernel4(float *layerGPU,float *tmp,int layer_size,int *pos,float *ini,float *fin){
	int idGlobal = threadIdx.x+(blockDim.x*threadIdx.y)+(blockDim.x*blockDim.y*blockIdx.x);
	if(idGlobal > layer_size-1) return;
	if(idGlobal != 0 && idGlobal != layer_size-1){
...
__global__ void kernel5(float *layerGPU,float *maximosGPU,int i,int layer_size,float *tmp,int tam,int *pos){
	int idGlobal = threadIdx.x+(blockDim.x*threadIdx.y)+(blockDim.x*blockDim.y*blockIdx.x);
...
__global__ void kernel6(float *maximosGPU,int *posicionesGPU,int i,float *tmp,int *pos,float *ini,float *fin,int layer_size,float *layerGPU){
	maximosGPU[i] = tmp[0];
...
__global__ void kernel7(float *tmp,int *pos,float *ini,float *fin){
	int idGlobal = threadIdx.x+(blockDim.x*threadIdx.y)+(blockDim.x*blockDim.y*blockIdx.x);
	tmp[idGlobal] = 0;
...
int main(int argc, char *argv[]) {
	...
	cudaMalloc(&ini,sizeof(float));
	cudaMalloc(&fin,sizeof(float));
	...
	cudaError_t errorDevCpyMax = cudaMemcpy(maximosGPU,maximos,sizeof(float)*num_storms,cudaMemcpyHostToDevice);
	cudaError_t errorDevCpyPos = cudaMemcpy(posicionesGPU,posiciones,sizeof(int)*num_storms,cudaMemcpyHostToDevice);

	float *tmpGPU;
	int *posGPU;

	cudaMalloc(&tmpGPU,sizeof(float)*layer_size-2);
	cudaMalloc(&posGPU,sizeof(int)*layer_size-2);
	...
	for( i=0; i<num_storms; i++) {
		...
		kernel3<<<numBlocks,numThreads>>>(layerGPU,layerCopyGPU,layer_size);

		kernel4<<<numBlocks,numThreads>>>(layerGPU,tmpGPU,layer_size,posGPU,ini,fin);

		for(int tam = layer_size-2; tam > 1; tam = tam/2){
			kernel5<<<numBlocks,numThreads>>>(layerGPU,maximosGPU,i,layer_size,tmpGPU,tam,posGPU);
		}

		kernel6<<<1,1>>>(maximosGPU,posicionesGPU,i,tmpGPU,posGPU,ini,fin,layer_size,layerGPU);

		kernel7<<<numBlocks,numThreads>>>(tmpGPU,posGPU,ini,fin);

...
void actualiza( float *layer, int k, int pos, float energia,int distancia,float atenuacion,float energia_k ) {
	/* 1. Calcular valor absoluto de la distancia entre el
		punto de impacto y el punto k de la capa */
	distancia = fabs(pos - k)+1;
...
int main(int argc, char *argv[]) {
	...
	/*Añadidos varios pragma omp*/

	/*	./client -u g24 -x 1r3CtRMN -q openmplb energy_v2.c -t 4 */  
	...
	*/
	...
	/* 4. Fase de bombardeos *//*La mas peligrosa*/

	int posicion,distancia;
	float energia,atenuacion,energia_k;
	...
	for( i=0; i<num_storms; i++) {
		...
		for( j=0; j<storms[i].size; j++ ) {
			...
			#pragma omp parallel for
			for( k=0; k<layer_size; k++ ) {
				actualiza( layer, k, posicion, energia,distancia ,atenuacion,energia_k);
			}
		}
		...
		for( k=1; k<layer_size-1; k++ ) {
			...
			if ( (layer[k] > layer[k-1] && layer[k] > layer[k+1]) &&  layer[k] > maximos[i] ) {
